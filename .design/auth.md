...
The `gswarm` CLI (running on the host) will look for authentication markers in the following order of precedence:
...
Once discovered, `gswarm` will inject these credentials into the agent runtime.
...
- During `gswarm init`, the template should *not* contain hardcoded keys.
...
4.  **CLI Flag**: Add a `--no-auth-propagation` flag to `gswarm start` for users who want to keep an agent entirely unauthenticated or manually provide keys.
